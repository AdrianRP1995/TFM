...
for file in $(ls ~/TFM/crispr/crispr_finds); do
  grep -A 1 "# 1 hits found" ~/TFM/blast/results/shmakov/$file.blast | grep "contigs" | cut -f 1,5 >> ~/TFM/blast/tables/shmakov_repeats_mismatches.tsv
done
#We extract a .tsv file with the number of mismatches


sed -i 's/_CRISPR/_CRISPR_/g' ~/TFM/blast/tables/shmakov_repeats_mismatches.tsv
...
sed -i 's/_CRISPR/_CRISPR_/g' ~/TFM/blast/tables/crispr_ws_repeats_mismatches.tsv
...
paste ~/TFM/blast/tables/shmakov_results_table.tsv ~/TFM/blast/tables/crispr_ws_repeats_results_table.tsv | cut -f 1,2,3,5,6 > ~/TFM/blast/tables/blast_definitive_results.tsv


rm ~/TFM/blast/tables/*shmakov* ~/TFM/blast/tables/*crispr* ~/TFM/blast/tables/*.tsve
